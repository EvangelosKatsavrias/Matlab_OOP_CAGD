...
switch obj.basisFunctionsType
    case 'NURBS'
        if any(obj.requestedDerivativesOrder) > 0
...
for parametricCoordinateIndex = 1:obj.numberOfParametricCoordinates
    if isempty(obj.MonoParametricBasisFunctions(parametricCoordinateIndex).evaluationsPerKnotPatch)
        obj.MonoParametricBasisFunctions(parametricCoordinateIndex).EvaluationSettings.setRequestedDerivativesOrder(obj.requestedDerivativesOrder(parametricCoordinateIndex));
        obj.MonoParametricBasisFunctions(parametricCoordinateIndex).evaluatePerKnotPatch;
    end
end
...
if strcmp(obj.basisFunctionsType, 'NURBS') || strcmp(obj.basisFunctionsType, 'BSpline')
    ...
    if strcmp(obj.basisFunctionsType, 'NURBS')
        for parametricCoordinatesIndex = 1:obj.Connectivities.numberOfParametricCoordinates
            if ~((isvector(obj.rationalControlPoints.weights)&&obj.MonoParametricBasisFunctions(parametricCoordinatesIndex).KnotVector.numberOfBasisFunctions == length(obj.rationalControlPoints.weights)) || (obj.MonoParametricBasisFunctions(parametricCoordinatesIndex).KnotVector.numberOfBasisFunctions == size(obj.rationalControlPoints.weights, parametricCoordinatesIndex)))
        ...
        if ~isvector(obj.rationalControlPoints.weights); obj.orientedWeights = permute(obj.rationalControlPoints.weights, obj.Connectivities.controlPointsCountingSequence); else obj.orientedWeights = obj.rationalControlPoints.weights; end
    end
...
temp = rationalBSplineBasisFunctions(obj.MonoParametricBasisFunctions.KnotVector, obj.MonoParametricBasisFunctions.EvaluationSettings, obj.rationalControlPoints.weights');
temp.evaluatePerKnotPatch;
